...
def get_net(input_shape=(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), load_weight_path=None, features=False, mal=False,summary=True) -> Model:
    inputs = Input(shape=(16, 16, 16, 1), name="input_1")
    x = inputs
...
def train(load_weights_path=None):
    ...
    for fold in range(k):
        ...
        callbacks_list = [keras.callbacks.EarlyStopping(monitor='acc', patience=200),
                          keras.callbacks.ModelCheckpoint(filepath="weight/3dcnn_16/cube_12vs45_32_{}flod".format(fold)+".h5", monitor='val_loss',save_best_only=True, ),
        ...
        model = get_net(load_weight_path="weight/3dcnn_16/cube_12vs45_32_{}flod".format(2)+".h5",summary=True)
