...
def train(load_weights_path=None):
    ...
    train_gen = data_generator(100, cube_and_y, False)
    """
    tmp = next(train_gen)
    for i in range(0, 100):
        cube_img = tmp[0][0][i].reshape(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1)
        mal=tmp[0][1][i][0]
        # cube_img_32 = cube_img[:, :, :, 0]
        cube_img_16=cube_img[8:24, 8:24, 8:24, 0]
        # cube_img_32 *= 255.
        cube_img_16 *= 255.
        # cube_img += MEAN_PIXEL_VALUE
        helpers.save_cube_img("img_16_" + str(i)+"_mal%s"%(mal) + ".png", cube_img_16, 4, 4)
        # helpers.save_cube_img("img_32_" + str(i) +"_mal%s"%(mal)+ ".png", cube_img_32, 4, 8)
    print("done")
    """

    for fold in range(k):
        print("Fold %s".format(fold))
        ...
        callbacks_list = [keras.callbacks.EarlyStopping(monitor='acc', patience=200),
                          keras.callbacks.ModelCheckpoint(filepath="weight/sudfnn_32/cube_chara_12vs45_32_{}flod".format(fold)+".h5", monitor='val_loss',
                                                          save_best_only=True, ),
        ...
        model = get_net(load_weight_path="weight/sudfnn_32/cube_chara_12vs45_32_{}flod".format(fold)+".h5",summary=True)
