...
def get_net(input_shape=(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), load_weight_path=None, features=False, mal=False,summary=True) -> Model:
    inputs = Input(shape=(32, 32, 32, 1), name="input_1")
    x = inputs
    ...
    out_class=Dropout(rate=0.4)(x)

    out_class = Dense(2, activation="softmax")(out_class)
...
def train(load_weights_path=None):
    ...
    for fold in range(k):
        ...
        callbacks_list = [keras.callbacks.EarlyStopping(monitor='acc', patience=200),
                          keras.callbacks.ModelCheckpoint(filepath="weight/3dcnn_32/cube_12vs45_32_{}flod".format(fold)+".h5", monitor='val_loss',save_best_only=True, ),
        ...
        history = model.fit_generator(train_gen, epochs=2, steps_per_epoch=35, validation_data=holdout_gen,
        ...
        model = get_net(load_weight_path="weight/3dcnn_32/cube_12vs45_32_第{}flod".format(2)+".h5",summary=True)
